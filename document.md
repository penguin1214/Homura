...
> One question about path integral form:
It is said that the most basic energy balance equation integrals over directions, and to ease sampling, we convert it to an integral over area. Further develop the integral to the **path integral form**. However, in `pbrt 14.5.3 incremental path construction`, it is said "At each vertex, the BSDF is sampled to generate a new **direction**". How to understand this inconsisteness?
A: The inconsistance is solved by converting the sampling of solid angle to sampling from area.
